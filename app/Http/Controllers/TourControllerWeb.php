...
class TourControllerWeb extends Controller
{
    ...
    public function listTestimonials($language)
    {
        $url = 'website.' . $language . '.testimonials';

        $tours = Testimonial::where('status',"approved")->paginate(10);
        
        
        return view($url)->with('testimonials', $tours);
    ...
    {//
        ...
        {
            $url = 'website.' . 'fr' . '.tours.decouverte.decouverte';
        }
        if($request->language == 'italian')
        {
            $url = 'website.' . 'it' . '.tours.decouverte.decouverte';
        }
        if($request->language == 'spanish')
        {
            $url = 'website.' . 'es' . '.tours.decouverte.decouverte';
        }
        if($request->language == 'english')
        {
            $url = 'website.' . 'en' . '.tours.decouverte.decouverte';
        }
        if($request->language == 'german')
        {
            $url = 'website.' . 'de' . '.tours.decouverte.decouverte';
        }
        ...
        $count = count($tour);
        return view($url)->with('tours', $tour)->with('count',$count)->with('language',$request->language)->with('tour_category',$tour_category);
    }
}
